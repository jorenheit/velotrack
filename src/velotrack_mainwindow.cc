...
void VeloTrackMainWindow::sendDownloadSignal()
{
    ...
    QEventLoop wait;
    connect(d_serial, SIGNAL(readyRead()), &timer, SLOT(start())); 
    connect(d_serial, SIGNAL(readyRead()), this, SLOT(transfer()));
    connect(&timer, SIGNAL(timeout()), &wait, SLOT(quit()));

    d_serial->write(QByteArray(1, TRANSFER_BUFFER));
    d_status->setText("Downloading data, please wait ...");
    wait.exec();
    d_status->setText("Download finished");
    disconnect(d_serial, SIGNAL(readyRead()), this, SLOT(transfer()));
    
    processRaw();
    saveData();
}

void VeloTrackMainWindow::transfer()
{
    while (d_serial->bytesAvailable())
        d_raw.append(d_serial->readAll());
}
